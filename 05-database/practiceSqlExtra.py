...
def showProductStockValue(cursor):
    output_format = """\
      ...
      STOCK VALUE = {:,.2f} """
    for i in cursor:
        print(output_format.format(i['productid'], i['productname'],
                                   i['stockvalue']))
        printLine()
...
def showInvoice(cursor):
    ...
    for idx, i in enumerate(cursor):
        if idx == 0:
            print(header_format.format('Order ID', i['orderid'], 'Order Date',
    ...
    vat = total * 0.07
    net_price = total + vat
    print(total_format.format('TOTAL PRICE', total, 'VAT (7%)', vat,
                              'NET PRICE', net_price))
    printLine(n=70)
...
def showCustomersBySales(cursor):
    ...
    print(header_format.format('Country', 'No.Of Order', 'NET Price',
                               'Price/Order'))
    printLine(n=70)
    for i in cursor:
        print(result_format.format(i['country'], i['number'], i['price'],
                                   i['po']))
...
if __name__ == '__main__':
    ...
    myDatabase = "Sqlite_Northwind.sqlite3"
